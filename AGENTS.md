...
- 复杂思考、方案和最终审查由 Sol `xhigh` 负责；边界明确且耗时的实施或大量简单只读分析按 [sol-luna-delivery](.codex/skills/sol-luna-delivery/SKILL.md) 交给 Luna。Luna 默认 `xhigh`，只有任务满足确定性降级条件时才显式降低强度；派发前必须提供完整、明确、可直接执行的任务包。
