...
- 先判断当前主代理：主代理为 Luna 时不启动 [sol-luna-delivery](.codex/skills/sol-luna-delivery/SKILL.md) 流程，直接由当前 Luna 处理；只有主代理为 Sol 时才评估并运行该流程。复杂思考、复杂实现、架构与根因判断、并发/事务/兼容性/安全/解析器/文件系统语义和最终审查由 Sol `xhigh` 负责。只有简单、边界冻结、低风险且可确定性验收的任务才交给 Luna；多文件或耗时实施默认 Luna `xhigh`，局部且只有少量明确分支可用 `high`，`medium` 仅用于确认无脑的机械任务，`low` 仅用于统计、格式调整和指定命令。难以判断时选 Sol；Sol 必须先完成并冻结直接可执行的详细设计，再仅将有界执行交给 Luna；派发前必须提供完整、明确、可直接执行的任务包。
