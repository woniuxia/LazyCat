...
- 未经用户明确要求不执行破坏性命令。
- 可视化辅助默认开启：涉及布局、交互、结构对比、流程图、信息架构或其他明显更适合视觉表达的内容时，默认先启用本地预览并在浏览器中展示；仅在用户明确拒绝、要求纯文本，或当前内容明显不适合可视化时回退到终端文字说明。
- 对技术方案、架构对比、交互设计、流程讲解等更适合视觉表达的内容，默认优先使用仓库现成的本地预览脚本启动浏览器展示：Windows 优先用 `scripts/start-server.ps1` / `scripts/stop-server.ps1`，兼容入口为 `scripts/start-server.sh` / `scripts/stop-server.sh`；展示内容可采用本地预览页、HTML 原型页、对比页面、流程图/线框图页面或浏览器内说明页。仅在用户明确要求纯文本，或当前内容明显更适合终端文本时回退到文字沟通。
