...
- 刷新路径的自动选中首条路由（无选中路由时）仅在路由表单非 dirty 时执行，避免保存项目等操作触发的刷新静默覆盖正在编辑的路由草稿。
...
- 延迟作用于命中路由的响应（含普通 OPTIONS 路由命中）；CORS 预检（204）不套用延迟，避免浏览器场景下预检 + 实际请求双重延迟。
...
5. Windows 下 accept 出的 socket 继承监听端的非阻塞模式，连接线程内先切回阻塞读并设置 10 秒读超时，消除原实现"请求字节未到达即 WouldBlock 丢弃连接"的潜在竞态。
