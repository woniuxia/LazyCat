...
| 类别 | 文件 | 行数 | 近 4 个月变更次数 |
|---|---|---|---|
| 前端 God 面板 | TodoPanel.vue | 2901 | 39 |
...
- `components/` 共 123 个文件基本平铺，但已有分域先例：`api-mock/`、`db/`、`common/`、`settings/`。域聚类明显：Pm* 18 个、Todo* 8 个、Api* 6 个、Spotlight* 5 个。
- Rust 侧有成熟拆分先例：pm.rs 拆为 pm / pm_today / pm_calendar / pm_matrix / pm_siyuan / pm_todo_link；子目录先例 `db_drivers/`。
...
5. **目录规则**：新子组件放 `components/<域>/`；拆哪个域就把该域既有文件一并 `git mv` 进去。composables/ 仅 24 个文件，保持平铺。
6. **避让约束**：
   - 不碰工作区进行中的 JsonTreeViewer 相关文件；每批开工前确认工作区干净。
   - `ApiWorkbenchPanel.vue` 拆分延后到 API Workbench UX 18 项落地之后（多标签改造会重构该面板）。
   - TodoPanel 拆分排在快速添加栏 plan（`docs/superpowers/plans/2026-07-04-todo-quick-add-plan.md`）实施之后。
...
| 3 | Todo 域：`components/todo/` + 拆 TodoPanel.vue + 拆 todo.rs | 快速添加栏 plan 已实施 | 大 |
| 4+ | 候选池逐批评估（见第 9 节） | 按当时热度×规模重排 | — |

## 5. 批次 0 详设：恢复 e2e 安全网

- **改法**：以 `'__TAURI_INTERNALS__' in window` 做 isTauri 探测；`getCurrentWindow()` 从 setup 顶层改为守卫后的惰性获取；无 Tauri 环境时相关调用（vault 失焦锁定、窗口控制）降级为 no-op。
- **涉及**：App.vue 中 vault 锁定/窗口控制调用点。
- **验收**：`pnpm test:e2e` 冒烟通过；桌面态 vault 失焦锁定与窗口行为不变。
...
- `git mv` 18 个 `Pm*` 文件至 `components/pm/`；同步更新 `tool-registry.ts` 动态 import 与各引用点。
...
| `pm/PmToolbar.vue` | 工具栏（视图切换/搜索/筛选/创建） | 模板 91-159 |
...
tools/api_workbench/
  ...
  response.rs   响应缓存 3 action + Office 预览
```

全部 31 个 action 与现有 match 分发点（原文件约 3131-3169 行）一一对应迁移；`list` 保留在 mod.rs。
...
**验收**：`cargo test`（含 api_workbench 相关测试，若有）+ 接口调试面板手工冒烟（发送/历史/环境/导出/响应预览）+ typecheck + e2e。

**协调注记**：后续实施 UX 18 项 plan 时，plan 中指向 `api_workbench.rs` 的路径引用按本节新结构对号入座。

## 8. 批次 3 概要：Todo 域

前提：快速添加栏 plan 已实施（届时 TodoPanel 结构已变），**批次启动时再按当时代码出行号级接缝清单**，本节只定框架：

- 目录搬迁：8 个 `Todo*` 文件 + 快速添加新组件迁入 `components/todo/`。
- TodoPanel.vue 抽取方向（按 2026-07-04 探索）：编辑器状态机（编辑/创建/dirty 检测）、调度字段（日期/提醒/重复）、PM 关联、筛选分组、CRUD 操作五个 composable。
- 有利条件：TodoPanel 已有组件测试（edit-focus、title-enter），拆分全程必须保持通过。
...
| TodoPanel 顺序 | 快速添加栏 plan 先行 | 拆分先行会作废已三审定稿的 plan |
| 目录策略 | 渐进分域（方案 B） | 一次性大搬家（方案 A）与 3 个待实施 plan 路径引用全面冲突；双线并行（方案 C）回归定位难 |
| Rust 拆分形态 | 目录化 `tools/api_workbench/` | 平铺 `api_workbench_*.rs` 会加剧 tools/ 42 文件的膨胀；db_drivers/ 已有子目录先例 |
