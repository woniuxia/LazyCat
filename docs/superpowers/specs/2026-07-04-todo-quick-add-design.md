...
- 日期速选控件：菜单项为 今天 / 明天 / 选日期…（弹日历）/ 清除日期；未选择时呈灰色占位“日期”，选中后显示相对文案（今天 / 明天 / MM-DD）并可单独清除（×）。
- 优先级速选控件：P0-P3。未手动选择时显示当前默认值（`priorityDefault`，见 2.5）并呈灰色占位态；用户手动选择后着色显示，着色即表示“显式指定”。
...
- 空标题回车忽略。
- 回车沿用面板既有的 `event.isComposing` 守卫：中文输入法组合态下的回车不触发创建。
- 创建请求进行中（in-flight）时再次回车忽略、不排队，避免快速连续回车产生重复任务；输入框保持可编辑，输入行在 pending 期间显示轻量加载指示。
