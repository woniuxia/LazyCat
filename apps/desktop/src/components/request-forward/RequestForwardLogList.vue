...
function setLogRowRef(id: number, element: Element | ComponentPublicInstance | null) {
...
function rowTabIndex(index: number): 0 | -1 {
  return index === selectedIndex.value || (selectedIndex.value < 0 && index === 0) ? 0 : -1;
}
...
<template>
  <div class="log-list" aria-live="polite">
    ...
    >
      ...
      >
        <span role="gridcell"
          ><b class="outcome" :class="log.error ? 'is-error' : 'is-success'">{{
            outcomeLabel(log)
          }}</b></span
        >
        <span class="method-cell" role="gridcell">{{
          log.protocol === "http" ? log.method || "—" : "—"
        }}</span>
        <span class="request-cell" role="gridcell"
          ><i>{{ log.protocol.toUpperCase() }}</i
          >{{ requestLabel(log) }}</span
        >
...
.log-table__row {
  ...
  grid-template-columns:
    58px 76px minmax(160px, 1.5fr) minmax(118px, 1fr) minmax(118px, 1fr)
    68px 68px 68px 84px;
  align-items: center;
}
.log-table__header {
...
.log-table__row.is-selected {
  background: #eaf3f8;
  box-shadow: inset 3px 0 0 var(--el-color-primary, #409eff);
}
.log-table__row:focus-visible {
  position: relative;
  z-index: 1;
...
@container forward-log-list (max-width: 820px) {
  .log-table__header,
  .log-table__row {
    grid-template-columns: 58px 64px minmax(140px, 1.5fr) minmax(96px, 1fr) 64px 68px 80px;
  }
  .log-table__header > :nth-child(5),
  .log-table__row > :nth-child(5),
  .log-table__header > :nth-child(7),
  .log-table__row > :nth-child(7) {
    display: none;
  }
}
...
@container forward-log-list (max-width: 620px) {
  .log-table__header,
  .log-table__row {
    grid-template-columns: 54px 62px minmax(0, 1fr) 80px;
  }
