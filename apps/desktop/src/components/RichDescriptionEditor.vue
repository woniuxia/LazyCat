    ...
    <div class="rte-toolbar" v-if="!disabled">
      <el-button-group size="small">
        <el-tooltip content="加粗 Ctrl+B" placement="top">
          <el-button :class="{ 'is-active': isActive('bold') }" @click="run((c) => c.toggleBold())"
            >B</el-button
          >
        </el-tooltip>
        ...
        <el-tooltip content="行内代码" placement="top">
          <el-button :class="{ 'is-active': isActive('code') }" @click="run((c) => c.toggleCode())"
            ><code>&lt;/&gt;</code></el-button
          >
        </el-tooltip>
      </el-button-group>
      ...
      <el-button-group size="small">
        ...
        <el-tooltip content="链接" placement="top">
          <el-button :class="{ 'is-active': isActive('link') }" @click="promptLink">Link</el-button>
        </el-tooltip>
      </el-button-group>
      <el-button-group size="small">
        <el-tooltip content="撤销 Ctrl+Z" placement="top">
          <el-button :disabled="!editor?.can().undo()" @click="run((c) => c.undo())">↶</el-button>
        </el-tooltip>
        <el-tooltip content="重做 Ctrl+Y" placement="top">
          <el-button :disabled="!editor?.can().redo()" @click="run((c) => c.redo())">↷</el-button>
        </el-tooltip>
      </el-button-group>
      <span class="rte-hint" v-if="uploadingCount > 0"> {{ uploadingCount }} 个附件上传中… </span>
    </div>
    <EditorContent class="rte-prose rte-editable" :editor="editor" />
    <RteFileRefMenu
