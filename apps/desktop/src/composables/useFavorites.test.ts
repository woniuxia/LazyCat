...
import { bootstrapFavoriteToolIds, normalizeFavoriteToolIds, TODO_TOOL_ID } from "./useFavorites";
  ...
  it("filters invalid ids and removes duplicates", () => {
    expect(
      normalizeFavoriteToolIds(
        [TODO_TOOL_ID, "formatter", "missing", TODO_TOOL_ID, 123],
        isRealToolId,
      ),
...
describe("bootstrapFavoriteToolIds", () => {
  it("seeds todo to the first position on first bootstrap", () => {
    expect(bootstrapFavoriteToolIds(["formatter", "snippets"], false, isRealToolId)).toEqual({
  ...
  it("does not duplicate todo and still marks bootstrap as completed", () => {
    expect(bootstrapFavoriteToolIds(["formatter", TODO_TOOL_ID], false, isRealToolId)).toEqual({
  ...
  it("does not re-add todo after the bootstrap flag is already set", () => {
    expect(bootstrapFavoriteToolIds(["formatter", "snippets"], true, isRealToolId)).toEqual({
