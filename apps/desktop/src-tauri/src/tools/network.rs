...
fn http_status_data() -> Vec<HttpStatus> {
    vec![
        HttpStatus { code: 100, name: "Continue", desc: "继续", usage: "客户端应继续发送请求体", causes: "" },
        HttpStatus { code: 101, name: "Switching Protocols", desc: "切换协议", usage: "WebSocket 升级", causes: "" },
        HttpStatus { code: 200, name: "OK", desc: "成功", usage: "请求成功", causes: "" },
        HttpStatus { code: 201, name: "Created", desc: "已创建", usage: "POST 创建资源成功", causes: "" },
        HttpStatus { code: 202, name: "Accepted", desc: "已接受", usage: "异步任务已接受处理", causes: "" },
        HttpStatus { code: 203, name: "Non-Authoritative Information", desc: "非权威信息", usage: "代理返回的修改后响应", causes: "" },
        HttpStatus { code: 204, name: "No Content", desc: "无内容", usage: "DELETE 成功，无返回体", causes: "" },
        HttpStatus { code: 206, name: "Partial Content", desc: "部分内容", usage: "Range 请求，断点续传", causes: "" },
        HttpStatus { code: 301, name: "Moved Permanently", desc: "永久重定向", usage: "资源永久迁移到新 URL", causes: "" },
        HttpStatus { code: 302, name: "Found", desc: "临时重定向", usage: "临时跳转到另一个 URL", causes: "" },
        HttpStatus { code: 303, name: "See Other", desc: "查看其他", usage: "POST 后重定向到 GET", causes: "" },
        HttpStatus { code: 304, name: "Not Modified", desc: "未修改", usage: "缓存有效，无需重新传输", causes: "" },
        HttpStatus { code: 307, name: "Temporary Redirect", desc: "临时重定向", usage: "保持请求方法的临时重定向", causes: "" },
        HttpStatus { code: 308, name: "Permanent Redirect", desc: "永久重定向", usage: "保持请求方法的永久重定向", causes: "" },
        HttpStatus { code: 400, name: "Bad Request", desc: "错误请求", usage: "请求参数错误或格式不正确", causes: "JSON 格式错误; 必填参数缺失; 参数类型不匹配; 请求体编码错误" },
        HttpStatus { code: 401, name: "Unauthorized", desc: "未授权", usage: "需要身份认证", causes: "Token 过期或无效; 未携带 Authorization 头; Cookie/Session 过期; OAuth 授权失败" },
        HttpStatus { code: 403, name: "Forbidden", desc: "禁止访问", usage: "服务器拒绝请求", causes: "无访问权限; IP 被封禁; CORS 策略拦截; CSRF Token 校验失败; 文件/目录权限不足" },
        HttpStatus { code: 404, name: "Not Found", desc: "未找到", usage: "请求的资源不存在", causes: "URL 路径拼写错误; 资源已被删除; 路由未注册; 后端接口未部署" },
        HttpStatus { code: 405, name: "Method Not Allowed", desc: "方法不允许", usage: "HTTP 方法不被支持", causes: "用 GET 请求了 POST 接口; 路由只允许特定方法; RESTful 方法不匹配" },
        HttpStatus { code: 406, name: "Not Acceptable", desc: "不可接受", usage: "无法满足 Accept 头要求", causes: "Accept 头与服务器响应格式不兼容" },
        HttpStatus { code: 407, name: "Proxy Authentication Required", desc: "需要代理认证", usage: "代理服务器要求认证", causes: "企业代理未配置认证; 代理凭证错误" },
        HttpStatus { code: 408, name: "Request Timeout", desc: "请求超时", usage: "服务器等待请求超时", causes: "客户端发送数据过慢; 网络不稳定; 大文件上传超时" },
        HttpStatus { code: 409, name: "Conflict", desc: "冲突", usage: "资源状态冲突", causes: "并发更新同一资源; 唯一约束冲突; 乐观锁版本不匹配" },
        HttpStatus { code: 410, name: "Gone", desc: "已删除", usage: "资源已永久删除", causes: "API 版本已下线; 资源被永久移除" },
        HttpStatus { code: 411, name: "Length Required", desc: "需要内容长度", usage: "缺少 Content-Length 头", causes: "未设置 Content-Length; 分块传输配置错误" },
        HttpStatus { code: 412, name: "Precondition Failed", desc: "前提条件失败", usage: "条件请求的前提不满足", causes: "If-Match/If-Unmodified-Since 条件不满足; ETag 不匹配" },
        HttpStatus { code: 413, name: "Payload Too Large", desc: "请求体过大", usage: "上传文件超过限制", causes: "上传文件超过 Nginx/后端限制; Body 大小超过配置; client_max_body_size 过小" },
        HttpStatus { code: 414, name: "URI Too Long", desc: "URI 过长", usage: "URL 超过服务器限制", causes: "GET 参数过多; 查询字符串过长; 应改用 POST" },
        HttpStatus { code: 415, name: "Unsupported Media Type", desc: "不支持的媒体类型", usage: "Content-Type 不被支持", causes: "Content-Type 与实际格式不匹配; 发送 JSON 但未设 application/json" },
        HttpStatus { code: 416, name: "Range Not Satisfiable", desc: "范围不满足", usage: "Range 请求超出范围", causes: "Range 头的字节范围超出文件实际大小" },
        HttpStatus { code: 422, name: "Unprocessable Entity", desc: "无法处理的实体", usage: "请求格式正确但语义错误", causes: "字段校验失败(邮箱格式、手机号等); 业务规则不满足; 枚举值不在允许范围内" },
        HttpStatus { code: 429, name: "Too Many Requests", desc: "请求过多", usage: "触发限流", causes: "API 调用频率超限; 爬虫/脚本触发防护; 缺少速率限制退避逻辑" },
        HttpStatus { code: 500, name: "Internal Server Error", desc: "服务器内部错误", usage: "服务器未知错误", causes: "后端代码抛出未捕获异常; 空指针/数组越界; 数据库连接失败; 依赖服务不可用" },
        HttpStatus { code: 501, name: "Not Implemented", desc: "未实现", usage: "服务器不支持该功能", causes: "接口尚未开发; 服务器不支持请求的 HTTP 方法" },
        HttpStatus { code: 502, name: "Bad Gateway", desc: "网关错误", usage: "上游服务器返回无效响应", causes: "后端服务崩溃/未启动; Nginx upstream 配置错误; 后端返回格式异常; 容器 OOM 被杀" },
        HttpStatus { code: 503, name: "Service Unavailable", desc: "服务不可用", usage: "服务器过载或维护中", causes: "服务正在部署/重启; 连接池耗尽; 线程池满; 熔断器打开; 服务器资源不足" },
        HttpStatus { code: 504, name: "Gateway Timeout", desc: "网关超时", usage: "上游服务器响应超时", causes: "后端处理时间过长; 数据库慢查询; 第三方接口超时; proxy_read_timeout 过小" },
        HttpStatus { code: 505, name: "HTTP Version Not Supported", desc: "HTTP 版本不支持", usage: "不支持请求的 HTTP 版本", causes: "客户端使用了不兼容的 HTTP 协议版本" },
    ]
}
...
fn chmod_calc(payload: &Value) -> Result<Value, String> {
    ...
    let bits: [bool; 9] = if mode == "symbolic" {
        ...
        [
            num & 0o400 != 0, num & 0o200 != 0, num & 0o100 != 0,
            num & 0o040 != 0, num & 0o020 != 0, num & 0o010 != 0,
            num & 0o004 != 0, num & 0o002 != 0, num & 0o001 != 0,
        ]
    };

    let numeric_val = (if bits[0] { 4 } else { 0 } + if bits[1] { 2 } else { 0 } + if bits[2] { 1 } else { 0 }) * 100
        + (if bits[3] { 4 } else { 0 } + if bits[4] { 2 } else { 0 } + if bits[5] { 1 } else { 0 }) * 10
