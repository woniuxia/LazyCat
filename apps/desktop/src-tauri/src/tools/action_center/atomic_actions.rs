...
use super::definitions::{
    definition, ActionTargetOption, BROWSER_PROFILE_LAUNCH, HOSTS_ACTIVATE,
...
fn composable_definition(action_type: &str) -> Result<super::definitions::ActionDefinition, String> {
    let definition = definition(action_type)
        .ok_or_else(|| format!("动作类型不存在: {action_type}"))?;
...
fn list_targets_with_conn(
    ...
    match action_type {
        ...
        BROWSER_PROFILE_LAUNCH => crate::tools::browser_profiles::list_action_targets().map(
            |rows| {
                rows.into_iter()
        ...
        REQUEST_FORWARD_START => {
            crate::tools::request_forward::list_action_targets_with_conn(conn).map(|rows| {
                rows.into_iter()
                    .map(|(id, label)| target_option(id, label, true, None))
                    .collect()
            })
...
fn resolve_target_with_conn(
    ...
    match action_type {
        ...
        BROWSER_PROFILE_LAUNCH => {
            ...
            let targets = list_targets_with_conn(conn, action_type)?;
        ...
        REQUEST_FORWARD_START => {
            let id = parse_numeric_target_id(target_id)?;
            crate::tools::request_forward::load_action_target_with_conn(conn, id).map(|target| {
                target.map(|label| target_option(id.to_string(), label, true, None))
            })
        }
        _ => unreachable!("combination action registry and target adapters must stay in sync"),
    }
...
fn validate_target_with_conn(
    ...
    let target = resolve_target_with_conn(conn, action_type, target_id)?
...
fn snapshot_target_with_conn(
    ...
    match resolve_target_with_conn(conn, action_type, target_id) {
...
mod tests {
    ...
    fn only_registered_safe_actions_are_composable() {
        ...
        assert!(!definition("release_package.run")
            .unwrap()
    ...
    fn rejects_unknown_actions_and_invalid_or_missing_targets() {
        ...
        assert!(list_targets_with_conn(&conn, "shell.run")
            .unwrap_err()
            .contains("动作类型不存在"));
        assert!(validate_target_with_conn(&conn, HOSTS_ACTIVATE, "not-an-id")
            .unwrap_err()
            .contains("目标 ID 无效"));
    ...
    #[test]
    fn snapshots_valid_unknown_and_unavailable_targets_without_hiding_identity() {
        let conn = test_conn();
        seed_hosts(&conn, 7, "开发");
        ...
        let unavailable = snapshot_target_with_conn(&conn, HOSTS_ACTIVATE, "8");
        assert_eq!(unavailable.action_label, "切换 Hosts");
        assert_eq!(unavailable.target_label, "8");
        assert!(unavailable.validation_error.unwrap().contains("目标不存在"));
