...
pub struct RetryExtraction {
    path: PathBuf,
}

impl RetryExtraction {
    pub fn path(&self) -> &Path {
        &self.path
    }
}

impl Drop for RetryExtraction {
    fn drop(&mut self) {
        let _ = fs::remove_dir_all(&self.path);
    }
}
...
pub fn extract_retry_zip(zip_path: &Path, destination: &Path) -> Result<RetryExtraction, String> {
    if !zip_path.is_file() {
        return Err(format!("重试 ZIP 不存在：{}", zip_path.display()));
    }
    if destination.exists() {
        return Err(format!("重试解压目录已存在：{}", destination.display()));
    }
    let file = File::open(zip_path)
        .map_err(|error| format!("打开重试 ZIP 失败（{}）：{error}", zip_path.display()))?;
    let mut archive = zip::ZipArchive::new(file)
        .map_err(|error| format!("读取重试 ZIP 失败（{}）：{error}", zip_path.display()))?;
    let mut paths = Vec::with_capacity(archive.len());
    for index in 0..archive.len() {
        let entry = archive
            .by_index(index)
            .map_err(|error| format!("读取重试 ZIP entry 失败：{error}"))?;
        if zip_entry_is_symlink(&entry) {
            return Err(format!("重试 ZIP 不能包含符号链接：{}", entry.name()));
        }
        let enclosed = entry
            .enclosed_name()
            .ok_or_else(|| format!("重试 ZIP 包含不安全路径：{}", entry.name()))?
            .to_path_buf();
        if enclosed.as_os_str().is_empty() {
            return Err("重试 ZIP 包含空路径".into());
        }
        let target = destination.join(&enclosed);
        if !target.starts_with(destination) {
            return Err(format!("重试 ZIP 路径逃逸：{}", entry.name()));
        }
        paths.push((enclosed, entry.is_dir()));
    }

    let guard = RetryExtraction {
        path: destination.to_path_buf(),
    };
    fs::create_dir_all(destination)
        .map_err(|error| format!("创建重试解压目录失败（{}）：{error}", destination.display()))?;
    for (index, (relative, is_dir)) in paths.into_iter().enumerate() {
        let target = destination.join(relative);
        if is_dir {
            fs::create_dir_all(&target).map_err(|error| {
                format!("创建重试 ZIP 目录失败（{}）：{error}", target.display())
            })?;
            continue;
        }
        if let Some(parent) = target.parent() {
            fs::create_dir_all(parent).map_err(|error| {
                format!("创建重试 ZIP 父目录失败（{}）：{error}", parent.display())
            })?;
        }
        ...
        let mut output = File::create(&target)
            .map_err(|error| format!("创建重试 ZIP 文件失败（{}）：{error}", target.display()))?;
        std::io::copy(&mut entry, &mut output)
            .map_err(|error| format!("解压重试 ZIP 文件失败（{}）：{error}", target.display()))?;
    }
    Ok(guard)
}
